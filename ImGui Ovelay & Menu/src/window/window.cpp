...
void SetupTheme() {
	...
	colors[ImGuiCol_Border] = ImVec4(0.15f, 0.15f, 0.16f, 1.00f);

	ImVec4 accent = ImVec4(0.40f, 0.35f, 0.90f, 1.00f);
	ImVec4 accentHover = ImVec4(0.50f, 0.45f, 1.00f, 1.00f);
...
void Overlay::StartRender()
{
	...
	if (GetAsyncKeyState(VK_INSERT) & 1) {
		...
		if (RenderMenu) {
			SetWindowLong(overlay, GWL_EXSTYLE, WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_TOOLWINDOW);
			SetForegroundWindow(overlay);
		}
		else {
			SetWindowLong(overlay, GWL_EXSTYLE, WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_TOOLWINDOW | WS_EX_TRANSPARENT);
		}
	}
